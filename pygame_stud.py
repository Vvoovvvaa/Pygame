...
try:
    icon = pygame.image.load('images/icon.png.png').convert_alpha()
...
def draw_gameplay():
    global player_health, bullets_left, money_count, bg_x
    if not pygame.mixer.get_busy():
        sound.play(-1)
    lose_sound.stop()
...
def keys():
    global player_x, player_y, player_anim_count
...
def jump():
    global player_y, jump_count, is_jump, player_anim_count
    if not is_jump:
        keys = pygame.key.get_pressed()
...
def money_deals():
    global money_count, gameplay, game_win
...
def game_win_screen():
    global money_count, bullets_left, game_win
...
def mouse():
    global gameplay, player_x, player_health, money_count, game_win
